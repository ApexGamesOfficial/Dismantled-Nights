...
const libraryLauncherButton = document.getElementById(
  "libraryLauncherButton"
);

const settingsSignInButton = document.getElementById(
...
const addLibraryButtons = document.querySelectorAll(
  ".add-library-button"
);

const removeLibraryButton = document.querySelector(
  ".remove-library-button"
);

const installGameButton = document.querySelector(
  ".install-game-button"
);

const gameDetailsButtons = document.querySelectorAll(
  ".game-details-button"
);
...
const accountModalTitle = document.getElementById(
  "accountModalTitle"
);

const accountModalText = document.getElementById(
  "accountModalText"
);

const apexAuthText = document.getElementById("apexAuthText");
const googleAuthText = document.getElementById("googleAuthText");

const apexAuthButton = document.getElementById("apexAuthButton");
...
document.addEventListener("keydown", (event) => {
  ...
  [accountModal, launcherModal, gameModal].forEach((modal) => {
...
authTabs.forEach((tab) => {
  tab.addEventListener("click", () => {
    ...
    if (mode === "signup") {
      accountModalTitle.textContent = "Create your account.";
      ...
      accountModalTitle.textContent = "Welcome back.";
...
apexAuthButton.addEventListener("click", () => {
  showToast(
...
installGameButton.addEventListener("click", () => {
  openModal(launcherModal);
...
function getLibrary() {
  try {
    const storedLibrary = JSON.parse(
      localStorage.getItem(STORAGE_KEYS.library)
    );
...
function isGameInLibrary(gameId) {
  return getLibrary().includes(gameId);
}
...
function addGameToLibrary(gameId) {
  ...
  if (library.includes(gameId)) {
    showToast("This game is already in your library.");
    return;
  }
...
function removeGameFromLibrary(gameId) {
  ...
  showToast("Game removed from your library.");
}

function renderLibrary() {
  const hasGame = isGameInLibrary(GAME_ID);
...
function updateAddButtons() {
  const added = isGameInLibrary(GAME_ID);

  addLibraryButtons.forEach((button) => {
    if (added) {
...
addLibraryButtons.forEach((button) => {
  button.addEventListener("click", () => {
    addGameToLibrary(GAME_ID);
  });
});

removeLibraryButton.addEventListener("click", () => {
  removeGameFromLibrary(GAME_ID);
});
...
effectsToggle.addEventListener("click", () => {
  const enabled =
    !effectsToggle.classList.contains("active");
